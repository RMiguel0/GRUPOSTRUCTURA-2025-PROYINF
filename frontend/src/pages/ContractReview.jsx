...
export default function ContractReview() {
  ...
  const fromState = state?.application || null;
  const identity = state?.identity || null; // opcional: viene desde IdentityCheck
  ...
  useEffect(() => {
    // Si venimos con datos reales, refrescamos el storage
    if (fromState) {
      try {
  ...
  // Política: Si IdentityCheck nos envió estado, respetamos su “allOk”
  // Si no hay identidad en el state (entraron directo), permitimos confirmar igual.
  const canConfirm = identity ? Boolean(identity.allOk) : true;
    ...
    <div className="max-w-3xl mx-auto p-6">
      ...
      {!fromState && !fromStorage && (
        <div className="mb-4 text-xs rounded-md border px-3 py-2">
          Estás viendo datos de ejemplo porque entraste directo a{" "}
          <code>/contract-review</code>. Envía el formulario en{" "}
          <code>/apply</code> para ver tu información real.
        </div>
      )}
      ...
      {identity && (
        <div className="mb-4 text-xs rounded-md border px-3 py-2">
          ...
          {identity.allOk ? (
            ...
            <span className="text-red-600">pendiente/incorrecta ❌</span>
          )}
        </div>
      )}
      ...
      <p className="text-xs text-gray-500 mt-6">
        ...
        Creado:{" "}
        {application?.meta?.createdAt
          ? new Date(application.meta.createdAt).toLocaleString()
          : "—"}
      </p>
    </div>
  );
